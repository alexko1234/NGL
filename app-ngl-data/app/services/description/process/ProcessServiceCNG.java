	...
	public void saveProcessTypes(Map<String, List<ValidationError>> errors) throws DAOException {
		...
		if(	!ConfigFactory.load().getString("ngl.env").equals("PROD") ){
			...
			l.add(DescriptionFactory.newProcessType("RNA Sequencing", "rna-sequencing", ProcessCategory.find.findByCode("library"),
					...
					Arrays.asList(getPET("ext-to-rna-sequencing",-1),getPET("lib-prep",0),getPET("pcr-and-purification",1),getPET("normalization-and-pooling",2), getPET("prepa-fc-ordered",3), getPET("illumina-depot",4) ), //ordered list of experiment type in process type
