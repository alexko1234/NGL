...
angular.module('commonsServices', []).
    	...
    	//EXAMPLE: <input type="date" ng-model="x" date-timestamp/> the value of x in the scope will be a timestamp
    	...
    	}).directive('btResize', ['$parse', '$document', '$window', '$filter', function($parse,$document, $window, $filter) {    	    	
    	    var BT_RESIZE_SRC_REGEXP=/in\s*([\w.()]+)\s*\|*\s*[\S*\s]*\s*\|*\s*[\S*\s]*$/; 
    	    var BT_RESIZE_LIMIT_REGEXP = /limitTo\s*\:\s*([\S*]+)\s*\|*[\s\S]*$/;
    	    var bt_items = [];
    	    var bt_limit = undefined;
    	    var bt_showOther = false;
    		...
    		return {
    	          ...
    	          var inputRepeat = iElement.attr("ng-repeat");
  		    var sourceResize = parseBtResizeSrc(inputRepeat);
  		    var limitResize = parseBtResizeLimit(inputRepeat);

  		    scope.$watch(sourceResize.bt_source, function(newValue, oldValue){    			
  			if(newValue && newValue.length > 0){
  			    bt_items = angular.copy(newValue);		    			    
  			    scope.bt_max = bt_items.length;  			   
  			}
  		    });

  		    scope.$watch(limitResize.bt_limit, function(newValue, oldValue){		    		     
  			if(angular.isUndefined(scope.$parent.$parent.bt_limitMin)){
  			    scope.$parent.$parent.bt_limitMin = oldValue;		    			 
  			} 

  			if(angular.isUndefined(scope.bt_min) && angular.isDefined(scope.$parent.$parent.bt_limitMin)){
  			    scope.bt_min = scope.$parent.$parent.bt_limitMin;		    			 
  			}   			

  			if(angular.isDefined(bt_limit) && bt_limit!=limitResize.bt_limit){
  			    scope.bt_min = scope.$parent.$parent.bt_limitMin;    			    
  			    bt_showOther = true;
  			}
  			bt_limit = limitResize.bt_limit;
  		    });

  		    scope.setResizeValue = function(value){		    		   
  			scope.$parent.$parent.bt_size = value;
  			$parse(limitResize.bt_limit).assign(scope, value);
  		    };

  		    function parseBtResizeSrc(input){
  			var match = input.match(BT_RESIZE_SRC_REGEXP);
  			if (!match) {
  			    throw new Error(
  				    "Expected typeahead specification in form of '_modelValue_ (as _label_)? for _item_ in _collection_'" +
  				    " but got '" + input + "'.");
  			}
  			return { bt_source: match[1]};
  		    };

  		    function parseBtResizeLimit(input){
  			var match = input.match(BT_RESIZE_LIMIT_REGEXP);
  			if (!match) {
  			    throw new Error(
  				    "Expected typeahead specification in form of '_modelValue_ (as _label_)? for _item_ in _collection_'" +
  				    " but got '" + input + "'.");
  			}
  			return { bt_limit: match[1]};
  		    };
    	          }  
    		}; 
    		},
    		...
    		controller: function($scope) {
    		    ...
    		    this.isResizable = function(maxValue){		        	     
    			return maxValue > $scope.bt_min;		        	     
    		    }; 		    

    		    this.isShowOther = function(){
    			return bt_showOther;
    		    };

    		    this.resetShowOther = function(){
    			bt_showOther = false;
    		    };
    		    ...
    		    this.getSizeItems = function(){
    			if(angular.isDefined($scope.$parent.$parent.bt_size)){
    			    return $scope.$parent.$parent.bt_size;
    			}else{
    			    return $scope.bt_min;
    			}
    		    }

    		    this.updateResizeValue = function(value) { 
    			$scope.setResizeValue(value);    			
    		    }; 		                   
    		}
    	    };
    	}]).directive('btResizeButtons', function() {   	    
    		...
    		scope : { },
    		template: '<div>'
    		    +'<div ng-hide="!resizable">' 
    		    +'<button  class="small-litte-icone-transparent" ng-click="resizeUp()" ng-hide="showAll"><i><b>...</b></i></button>'
    		    +'<button  class="small-litte-icone-transparent" ng-click="resizeDown()" ng-show="showAll"><i class="fa fa-minus-square-o"></i></button>'
    		    +'</div>'
    		    +'</div>',		  

    		    link: function(scope, element, attrs, ctrl){    		    

    			if(angular.isUndefined(scope.maxValue)){
    			    scope.maxValue = ctrl.getMaxValue();
    			}

    			if(angular.isUndefined(scope.minValue)){
    			    scope.minValue = ctrl.getMinValue();
    			}

    			if(angular.isUndefined(scope.resizable)){
    			    scope.resizable = ctrl.isResizable(scope.maxValue);
    			}

    			var sizeItems = ctrl.getSizeItems();   	    
    			scope.showAll = sizeItems==scope.maxValue;      		    
    			...
    			scope.resizeUp = function (){ 			
    			    ...
    			    ctrl.updateResizeValue(scope.maxValue);    			
    			}; 
    			...
    			scope.resizeDown = function (){
    			    ...
    			    ctrl.updateResizeValue(scope.minValue);    			
    			}; 

    			scope.resetValue = function(){
    			    scope.maxValue = ctrl.getMaxValue();
    			    scope.minValue = ctrl.getMinValue();
    			    sizeItems = ctrl.getSizeItems();   	    
    			    scope.showAll = sizeItems==scope.maxValue; 
    			};
